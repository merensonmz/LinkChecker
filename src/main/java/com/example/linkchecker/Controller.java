...
public class Controller {
    ...
    private Set<String> addedUrls = new HashSet<>();


    private Stage mainStage;

    private List<String> urlsInTime = new ArrayList<>();
    ...
    private void showPopup() {
        ...
        ListView<String> listViewPopUp = new ListView<>();
        ...
        addButton.setOnAction(event -> {
            ...
            if (!url.isEmpty()) {
                urlsInTime.add(url);
                listViewPopUp.getItems().add(url);
                textField.clear();
            }
        });
        deleteButton.setOnAction(event -> {
            String selectedUrl = listViewPopUp.getSelectionModel().getSelectedItem();
            if (selectedUrl != null) {
                addedUrls.remove(selectedUrl); // Delete the URL from the Set
                listViewPopUp.getItems().remove(selectedUrl); // Delete the URL from the ListView
            }
        });

        popupContent.getChildren().addAll(textField, addButton, deleteButton, listViewPopUp);

        // Create the popup window
        Stage popupStage = new Stage();
        popupStage.initOwner(mainStage);
        popupStage.setTitle("Add URL");
        popupStage.setScene(new Scene(popupContent, 300, 200));
        popupStage.show();
    }
    ...
    private long getTargetTime() {
        LocalTime targetTime = LocalTime.of(14, 11);
